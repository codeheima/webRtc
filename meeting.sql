...
CREATE TABLE IF NOT EXISTS `appcc_meeting`(
	...
	`preStartTime`	DATETIME   COMMENT '预开始时间',
	`preEndTime`	DATETIME    COMMENT '预结束时间',
	`startTime` DATETIME    COMMENT '会议开始时间',
	`endTime`	DATETIME   COMMENT '会议结束时间',
	`createAt`	timestamp NOT NULL DEFAULT now() COMMENT '会议创建时间',
	PRIMARY KEY (`id`)
